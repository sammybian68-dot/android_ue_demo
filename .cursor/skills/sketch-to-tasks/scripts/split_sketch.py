...
Output: A directory structure ready for AI consumption:
    ...
    │   └── ...
    └── preview/               (symlink or copy from source)
"""
...
def collect_referenced_slices(artboard: dict) -> list[str]:
    slices = []
...
def run(input_path: str, output_dir: str):
    ...
    with open(os.path.join(output_dir, "artboard_index.json"), "w", encoding="utf-8") as f:
